...
// Helper function to check local pro status
const checkLocalProStatus = (userEmail) => {
  if (import.meta.env.VITE_FIREBASE_PROJECT_ID === 'your_project_id_here') {
    const proUser = localStorage.getItem('skillRealms_pro_user');
    return proUser === userEmail;
  }
  return false;
};
  ...
  const handleUnlockCode = async () => {
    ...
    // Local bypass for testing when Firebase permissions are not configured
    if (import.meta.env.VITE_FIREBASE_PROJECT_ID === 'skillway-1125b') {
      localStorage.setItem('skillRealms_pro_user', user.email);
      localStorage.setItem('skillRealms_pro_code', normalized);
      localStorage.setItem('skillRealms_pro_timestamp', new Date().toISOString());
      setCodeStatus('Pro unlocked successfully! (Local mode) Returning to dashboard...');
    ...
    try {
      ...
      if (!snap.empty && snap.docs[0].data().is_pro) {
        setCodeStatus('You already have Pro access! Redirecting to dashboard...');
        setTimeout(() => navigate('/dashboard'), 1500);
        return;
      }
      ...
      console.error('Error details:', {
        ...
        message: err.message,
        email: user?.email,
        codeEntered: normalized,
        expectedCode: 'unlockproforfeatures',
        timestamp: new Date().toISOString()
      });
      
      if (err.code === 'permission-denied') {
        setCodeStatus('Permission denied. Please check your Firebase permissions and try again.');
      } else if (err.code === 'unavailable') {
        setCodeStatus('Service temporarily unavailable. Please try again in a moment.');
      } else if (err.code === 'deadline-exceeded') {
        setCodeStatus('Request timed out. Please check your connection and try again.');
      } else if (err.code === 'resource-exhausted') {
        setCodeStatus('Firebase quota exceeded. Please try again later.');
      } else if (err.code === 'not-found') {
        setCodeStatus('User profile not found. Please try logging out and back in.');
      } else if (err.code === 'failed-precondition') {
        setCodeStatus('Firebase configuration error. Please try again.');
      } else if (err.code === 'already-exists') {
        setCodeStatus('Profile already exists. Please try logging out and back in.');
      } else if (err.code === 'cancelled') {
        setCodeStatus('Operation cancelled. Please try again.');
      } else {
        setCodeStatus(`Error: ${err.message || 'Could not unlock Pro right now. Please try again.'}`);
      }
    } finally {
      setIsUnlocking(false);
    }
