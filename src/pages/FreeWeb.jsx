...
export default function FreeWeb() {
  ...
  const isPro = userProfile?.is_pro || (import.meta.env.VITE_FIREBASE_PROJECT_ID === 'skillway-1125b' && user?.email && localStorage.getItem('skillRealms_pro_user') === user.email);
  const hasPermanentAccess = isPro || isGEDComplete;
  const freeWebDuration = import.meta.env.DEV ? 999999 : 1800; // Unlimited for development, 30 minutes for production

  useEffect(() => {
    if (!user) return;

    // In development, always grant access to everyone
  ...
  const tabs = [
    { id: 'proxy', label: 'Web Browser', icon: Globe, url: 'https://www.google.com/' },
    { id: 'vps', label: 'VPS Desktop', icon: Globe, url: 'about:blank' },
    { id: 'games', label: 'Educational Games', icon: Gamepad2, url: '/games' },
  ];
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className={`flex items-center gap-2 px-4 py-2 rounded-xl ${isPro || isGEDComplete ? 'bg-green-50' : 'bg-blue-50'}`}>
              {isPro || isGEDComplete ? <Infinity className="w-5 h-5 text-green-600" /> : <Clock className="w-5 h-5 text-blue-600" />}
            ...
            {currentTabConfig.id === 'games' ? (
              <div className="w-full h-full flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-gray-600 mb-6">Use the app games page for the real UI experience.</p>
                  <button
                    onClick={() => navigate('/games')}
