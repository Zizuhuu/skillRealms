...
// ---- REPLACE THIS with your actual Groq API key in .env ----
...
function safeJsonParse(raw) {
  ...
  console.log('Parsing JSON, input type:', typeof raw, 'length:', trimmed.length);
  console.log('First 100 chars:', trimmed.substring(0, 100));

  // Simple, reliable parser that works
  const parseWithNewlines = (jsonStr) => {
    try {
      // First, try normal JSON.parse
      return JSON.parse(jsonStr);
    } catch (error) {
      console.log('Normal JSON.parse failed, using simple parser');
      ...
      // Extract title - handle multiline and complex titles
      let titleMatch = jsonStr.match(/"title":\s*"([^"]+)"/);
      if (!titleMatch) {
        // Try multiline title pattern
        titleMatch = jsonStr.match(/"title":\s*"([\s\S]*?)"/);
      }
      if (titleMatch) {
        result.title = titleMatch[1].replace(/\\n/g, ' ').replace(/\s+/g, ' ').trim();
      }
      
      // Extract reading (simple approach)
      const readingMatch = jsonStr.match(/"reading":\s*"([^"]+)"/);
      if (readingMatch) {
        result.reading = readingMatch[1];
      }
      
      // Extract questions (simple approach)
      const questionsMatch = jsonStr.match(/"questions":\s*\[([\s\S]*?)\]/);
      console.log('Questions array match:', !!questionsMatch);
      if (questionsMatch) {
        ...
        // Simple approach: split by },{ and clean up each question object
        let questionMatches = questionsText.split(/},{/).map((q, index, array) => {
          // Add back the braces that were removed by split
          if (index === 0) q = q.replace(/^\s*\{/, '');
          if (index === array.length - 1) q = q.replace(/\}\s*$/, '');
          return '{' + q + '}';
        }).filter(q => q.includes('"question"'));
        
        console.log('Question matches found:', questionMatches?.length || 0);
        if (questionMatches) {
          console.log('Processing', questionMatches.length, 'question objects');
          for (let i = 0; i < questionMatches.length; i++) {
            const qText = questionMatches[i];
            console.log(`Question ${i + 1} text sample:`, qText.substring(0, 100));
            const question = {
              question: '',
              options: [],
              correct: 0,
            ...
            const optionsMatch = qText.match(/"options":\s*\[([\s\S]*?)\]/);
            if (optionsMatch) {
              const opts = optionsMatch[1].match(/"([^"]+)"/g);
              if (opts) {
                question.options = opts.map(o => o.replace(/"/g, '').replace(/\\n/g, ' ').trim());
                console.log('Extracted options:', question.options);
              }
            }
            
            // Handle both numeric and string correct answers
            let correctMatch = qText.match(/"correct":\s*(\d+)/);
            if (!correctMatch) {
              correctMatch = qText.match(/"correct":\s*"([^"]+)"/);
              if (correctMatch) {
              ...
              question.correct = parseInt(correctMatch[1]);
            }
            
            const expMatch = qText.match(/"explanation":\s*"([\s\S]*?)"/);
            if (expMatch) question.explanation = expMatch[1].replace(/\\n/g, ' ').replace(/\s+/g, ' ').trim();
            
            // If no explanation, try to get hint or description
            if (!question.explanation) {
              const hintMatch = qText.match(/"hint":\s*"([^"]+)"/);
              if (hintMatch) question.explanation = hintMatch[1];
            }
      ...
      // If we couldn't extract questions, create a simple lesson from the text
      if (result.questions.length === 0) {
        console.log('No questions found, creating simple lesson');
        result.reading = jsonStr.substring(0, 1000); // First 1000 chars as reading
        
        // Create simple questions from the content
        result.questions = [
          {
      ...
      console.log('Simple parser created lesson with', result.questions.length, 'questions');
      return result;
    }
  };
  ...
  try {
    ...
    console.log('Direct JSON parse failed, trying fenced code blocks');
  }

  // Try to extract JSON from markdown code blocks
  const fencedMatch = trimmed.match(/```(?:json)?\s*([\s\S]*?)\s*```/i);
  if (fencedMatch && fencedMatch[1]) {
    console.log('Found fenced code block, attempting to parse');
    let jsonStr = fencedMatch[1].trim();
    console.log('Extracted JSON length:', jsonStr.length);
    
    try {
      const result = parseWithNewlines(jsonStr);
      if (result) {
        console.log('Fenced JSON parse successful with custom parser');
        return result;
      }
    } catch (error) {
      console.log('Fenced JSON parse failed:', error.message);
    }
  }
  ...
  if (firstBrace !== -1 && lastBrace > firstBrace) {
    ...
    console.log('Trying brace extraction, length:', jsonStr.length);
    
    try {
      const result = parseWithNewlines(jsonStr);
      if (result) {
        console.log('Brace extraction parse successful with custom parser');
        return result;
      }
    } catch (error) {
      console.log('Brace extraction parse failed:', error.message);
    }
  }
...
async function generateAILesson(subject, lessonNumber, isProUser = false) {
    // Re-enable caching with proper cache key
    ...
    // Use multiple factors for better topic variation
    const now = new Date();
    const dayOfYear = Math.floor((now - new Date(now.getFullYear(), 0, 0)) / 86400000);
    const hour = now.getHours();
    const minute = now.getMinutes();
    const timeBasedSeed = dayOfYear + hour + minute;
    
    // Create more variation using lesson number, time, and random factor
    const topicIndex = ((lessonNumber - 1) * 7 + timeBasedSeed + Math.floor(Math.random() * 3)) % topics.length;
    const todayTopic = topics[topicIndex];

    const canUseGroq = await checkGroqKey();
    if (!canUseGroq) {
      return getFallbackLesson(subject);
    }

    // Progressive difficulty based on lesson number
    const difficultyMultiplier = Math.min(1 + (lessonNumber * 0.05), 2.5); // Increases by 5% per lesson, max 2.5x difficulty
    const questionComplexity = lessonNumber <= 10 ? 'basic' : lessonNumber <= 20 ? 'intermediate' : 'advanced';
    const readingLevel = lessonNumber <= 10 ? '6th-8th grade' : lessonNumber <= 20 ? '9th-10th grade' : '11th-12th grade';

    // Make questions harder each day based on current date
    const dailyDifficultyMultiplier = 1 + (dayOfYear * 0.01); // Increases by 1% per day of year
    const finalDifficultyMultiplier = difficultyMultiplier * dailyDifficultyMultiplier;

    const prompt = `Create a complete GED lesson about ${todayTopic}.

Return ONLY this JSON format:
{
  "title": "${todayTopic} GED Practice",
  "reading": "In ${todayTopic}, students learn fundamental concepts. This passage covers essential knowledge needed for the GED test, with clear explanations and examples.",
  "questions": [
    {
      "question": "Complete question text here",
      "options": ["Option A", "Option B", "Option C", "Option D"],
      "correct": 0,
      "explanation": "Complete explanation here"
    }
  ]
}

YOU MUST:
- Generate exactly ${isProUser ? 30 : 5} complete questions
- Each question must have: complete text, 4 options, correct number, explanation
- Do NOT cut off questions or stop early
- Ensure JSON is properly formatted and complete
- Use reading level: ${readingLevel}
- Use difficulty: ${questionComplexity}
...
Complete the entire lesson before responding.`;
    ...
    try {
      ...
      if (result && typeof result === 'object') {
        ...
        console.warn('API error response; using fallback', json.error);
        return { ...getFallbackLesson(subject), __fallbackReason: `API error: ${json.error.message || 'Unknown error'}` };
      } else if (json?.detail) {
      ...
      if (result?.title && result?.reading) {
        // Check if lesson is incomplete (contains truncated JSON or incomplete questions)
        const isLessonIncomplete = (text) => {
          if (!text) return true;
          
          // Check for incomplete JSON structure
          if (text.includes('"question":') && !text.includes('"explanation":')) {
            return true;
          }
          ...
          for (const pattern of incompletePatterns) {
            ...
            if (lastIndex !== -1) {
              const afterPattern = text.substring(lastIndex + pattern.length);
              // If there's no complete closing after the pattern, it's incomplete
              if (!afterPattern.includes('"') || afterPattern.includes('for,') || afterPattern.includes('for ')) {
                return true;
              }
            }
          }
        ...
        // Check if the lesson content is incomplete
        const readingIncomplete = isLessonIncomplete(result.reading);
        const hasIncompleteQuestions = result.questions && result.questions.some(q => 
          !q.question || !q.options || q.question.includes('for,') || q.question.includes('for ')
        );
        
        if (readingIncomplete || hasIncompleteQuestions) {
        ...
        if (!result.questions || !Array.isArray(result.questions) || result.questions.length === 0) {
          ...
          result.questions = result.questions.map((q, index) => {
            if (!q.question || !q.options || !Array.isArray(q.options) || q.options.length < 4) {
              console.warn(`Question ${index + 1} is incomplete, replacing with fallback`);
              return getFallbackLesson(subject).questions[index % getFallbackLesson(subject).questions.length];
            }
            
            // Ensure correct is a number
        ...
        const targetQuestionCount = isProUser ? 30 : 5;
        if (result.questions.length < targetQuestionCount) {
          const fallbackQuestions = getFallbackLesson(subject).questions;
          while (result.questions.length < targetQuestionCount) {
            result.questions.push({
              ...fallbackQuestions[result.questions.length % fallbackQuestions.length],
              question: `${result.questions.length + 1}. ${fallbackQuestions[result.questions.length % fallbackQuestions.length].question}`
            });
          }
        }
  ...
  if (showReading) {
    return (
      <div className="space-y-6">
        ...
        <VideoLesson subject={subject} lessonContent={safeData} />
