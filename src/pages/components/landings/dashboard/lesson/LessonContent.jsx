...
const DEFAULT_GROQ_MAX_TOKENS = 7600;
const DEFAULT_GROQ_TEMPERATURE = 0.5;
...
function safeJsonParse(raw) {
  ...
  const tryParse = (text) => {
    if (!text) return null;
    try {
      return JSON.parse(normalizeJson(text));
    } catch (error) {
      return null;
    }
  };
  ...
  const fallback = { title: 'AI Generated Lesson', reading: '', questions: [] };
  const titleMatch = trimmed.match(/"title"\s*:\s*"([^"]+)"/);
  if (titleMatch) fallback.title = titleMatch[1].replace(/\n/g, ' ').trim();
  const readingMatch = trimmed.match(/"reading"\s*:\s*"([^"]+)"/);
  if (readingMatch) fallback.reading = readingMatch[1].replace(/\n/g, ' ').trim();

  const questionsMatch = trimmed.match(/"questions"\s*:\s*\[([\s\S]*?)\]/);
  if (questionsMatch) {
    const questionItems = questionsMatch[1].split(/\}\s*,\s*\{/).map((item, index, arr) => {
      let qText = item;
      if (index === 0) qText = qText.replace(/^\s*\{/, '');
      if (index === arr.length - 1) qText = qText.replace(/\}\s*$/, '');
      return `{${qText}}`;
    });

    for (const item of questionItems) {
      const questionMatch = item.match(/"question"\s*:\s*"([^"]+)"/);
      const optionsMatch = item.match(/"options"\s*:\s*\[([\s\S]*?)\]/);
      if (!questionMatch || !optionsMatch) continue;

      const options = [...optionsMatch[1].matchAll(/"([^"]+)"/g)].map(m => m[1].replace(/\n/g, ' ').trim());
      const explanationMatch = item.match(/"explanation"\s*:\s*"([\s\S]*?)"/);
      fallback.questions.push({
        question: questionMatch[1].replace(/\n/g, ' ').trim(),
        options,
        correct: 0,
        explanation: explanationMatch ? explanationMatch[1].replace(/\n/g, ' ').trim() : 'Explanation not available.'
      });
    }
  }

  return fallback.questions.length ? fallback : null;
}
