...
export default function MathBlitzGame() {
  ...
  const handleGameOver = () => {
    ...
    // Mark 10/10 if player answered 10 questions correctly without missing any
    if (score >= 1000 && lives === 3) {
      try { localStorage.setItem('game_math_blitz_10_10', '1'); } catch {};
    }
  };
  ...
  return (
    <div className="min-h-[80vh] p-6 flex flex-col max-w-lg mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex gap-1">{[1,2,3].map(i => <Heart key={i} className={`w-7 h-7 ${i <= lives ? 'text-red-500 fill-red-500' : 'text-gray-500'}`} />)}</div>
        <div className="text-center"><p className="text-yellow-300 text-3xl font-black">{score}</p><p className="text-gray-400 text-xs">SCORE</p></div>
        <div className="text-right"><p className="text-white font-bold">LVL {level}</p><Zap className="w-5 h-5 text-cyan-400 ml-auto" /></div>
      </div>
