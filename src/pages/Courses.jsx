  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        ...
        const isProLocal = import.meta.env.VITE_FIREBASE_PROJECT_ID === 'skillway-1125b' && 
          localStorage.getItem('skillRealms_pro_user') === user.email;
        setUserProfile({ is_pro: isProLocal });
      }
    });
    return unsubscribe;
  }, []);
      ...
      {expanded && (
        <div className={`border-t ${course.borderColor} ${course.bg} p-5 space-y-4`}>
          ...
          {isProCourse && !isPro ? (
            ...
            <Link to={`/lesson?subject=${course.id}`}><Button className={`w-full bg-gradient-to-r ${course.color} text-white rounded-xl h-12 hover:opacity-90`}><Play className="w-4 h-4 mr-2 fill-white" /> {isProCourse && isPro ? 'Access Pro Course' : 'Start Learning'}</Button></Link>
          )}
        </div>
      )}
    </div>
